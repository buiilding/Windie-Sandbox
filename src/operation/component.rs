//! Installed tool-component lifecycle operations.
//!
//! These operations persist component state and run explicit health checks.
//! The public function names still use `provider` for API and database
//! compatibility, but the implementation now belongs to the component
//! lifecycle boundary.
...
fn prepare_provider(manifest: &ProviderManifest) -> Result<()> {
    if !crate::tool::ProviderPlatform::supports_current(&manifest.platforms) {
...
mod tests {
    use super::*;
    use crate::tool::ProviderPlatform;
