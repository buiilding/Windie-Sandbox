...
/// Small, typed signal emitted only by the tray-notification development
...
pub(super) enum TrayTestNotification {
    /// Exercises the notification shown after an assistant response finishes.
    AssistantCompleted,
}

impl TrayTestNotification {
    /// Returns the stable SSE event name understood by the native tray.
    const fn event_name(self) -> &'static str {
        match self {
            Self::AssistantCompleted => "tray.assistant_completed",
        }
    }
}
...
pub(super) struct TrayTestNotificationResponse {
    /// Number of connected tray SSE receivers that accepted the signal.
    tray_receivers: usize,
}

/// Announces a volatile assistant-completed test signal to connected tray
/// clients. A missing tray receiver is valid: this endpoint is a manual test
/// aid and does not retain or retry signals.
pub(super) async fn report_assistant_completed_for_tray(
    State(state): State<ApiState>,
) -> (StatusCode, Json<TrayTestNotificationResponse>) {
    let tray_receivers = announce_test_assistant_completion(&state.tray_test_notifications);
    (
        StatusCode::ACCEPTED,
        Json(TrayTestNotificationResponse { tray_receivers }),
    )
}

/// Publishes the manual probe without making delivery a runtime guarantee.
fn announce_test_assistant_completion(
    sender: &tokio::sync::broadcast::Sender<TrayTestNotification>,
) -> usize {
    sender
        .send(TrayTestNotification::AssistantCompleted)
        .unwrap_or_default()
}

/// Streams development-only tray signals to local tray processes.
///
/// The stream has no persistence or replay cursor by design. Production tray
...
pub(super) async fn tray_test_notifications(
    State(state): State<ApiState>,
) -> Sse<impl futures_util::Stream<Item = std::result::Result<Event, Infallible>>> {
    let receiver = state.tray_test_notifications.subscribe();
            ...
            match receiver.recv().await {
                Ok(notification) => {
                    let data = serde_json::to_string(&notification).unwrap_or_else(|error| {
                        serde_json::json!({
                            "error": format!("failed to serialize tray test notification: {error}")
                        })
                        .to_string()
                    });
                ...
                Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {
                    ...
                    // the next message rather than terminating the tray link.
                }
...
mod tests {
    ...
    fn assistant_completed_test_signal_has_a_namespaced_event_name() {
        assert_eq!(
            TrayTestNotification::AssistantCompleted.event_name(),
    ...
    async fn assistant_completed_test_signal_is_delivered_to_live_receivers() {
        ...
        assert!(matches!(
            receiver.recv().await,
            Ok(TrayTestNotification::AssistantCompleted)
        ));
    }
}
